...
fn test_replace() {
  let mut storage = NodeField::<Option<i32>>::new();
  let idx = storage.push(Some(1));

  // Replace and verify old data is returned
  let old = storage.replace(idx, Some(2)).unwrap();
  assert_eq!(old, Some(1));

  // Verify new data is in place
  assert_eq!(*storage.get(idx).unwrap(), Some(2));
}
...
fn test_reference_counting() {
  ...
  assert_eq!(storage.remove_ref(idx).unwrap().unwrap(), Some(1));
...
fn test_trim_all_free() {
  let mut storage = NodeField::<Option<u32>>::new();
