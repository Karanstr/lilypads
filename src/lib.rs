...
pub trait Nullable: Eq + Sized {
  ...
  /// Checks whether self is equal to Self::NULLVALUE
  fn is_null(&self) -> bool { self == &Self::NULLVALUE }
  /// Replaces self with Self::NULLVALUE and returns Some(old_self), or None if is already Self::NULLVALUE
  fn take(&mut self) -> Option<Self> {
    if self.is_null() { None } else {
      Some(std::mem::replace(self, Self::NULLVALUE))
    }
  }
}

impl<T> Nullable for Option<T> where T: Eq { const NULLVALUE:Self = None; }
...
pub struct NodeField<T:Clone + Nullable> {
  /// List of all data stored within this structure
  data : Vec< T >,
  /// A reference count for each data slot
  refs : Vec< Option<NonZeroUsize> >,
}

// Private methods
impl<T:Clone + Nullable> NodeField<T> {
  ...
  fn mark_reserved(&mut self, idx:usize) { self.refs[idx] = Some(NonZeroUsize::new(1).unwrap()); }
...
impl<T:Clone + Nullable> NodeField<T> {
  ...
  pub fn get(&self, idx:usize) -> Result<&T, AccessError> {
    if idx >= self.data.len() { return Err(AccessError::FreeMemory(idx)) }
    if self.data[idx].is_null() { Err(AccessError::FreeMemory(idx)) } else { Ok(&self.data[idx]) }
  }
  ...
  pub fn get_mut(&mut self, idx:usize) -> Result<&mut T, AccessError> {
    if idx >= self.data.len() { return Err(AccessError::FreeMemory(idx)) }
    if self.data[idx].is_null() { Err(AccessError::FreeMemory(idx)) } else { Ok(&mut self.data[idx]) }
  }
  ...
  pub fn remove_ref(&mut self, idx:usize) -> Result<Option<T>, AccessError> {
    if let Some(Some(count)) = self.refs.get_mut(idx) {
      Ok( if let Some(new_count) = NonZeroUsize::new(usize::from(*count) - 1) {
        *count = new_count;
        None
      } else { Some(self.release(idx)) } )
    } else { Err(AccessError::FreeMemory(idx)) }
  }
  ...
  pub fn refs(&self) -> &Vec< Option< NonZeroUsize > > { &self.refs }
}
